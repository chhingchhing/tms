...
//                    'unit_price' => $cur_item_info->price_one,
...
////            $cur_item_info = $this->massage->get_info($sale_item_row['item_massage_id']);
////            $item_data = array('quantity' => $sale_item_row['quantity_purchased']);
////            $this->massage->save($item_data, $sale_item_row['id_order_massage']);
//
//            $sale_remarks = 'CGATE ' . $sale_id;
//            $inv_data = array
//                (
...
//        $query = $this->db->query('SELECT massage_name, time_in, time_out, unit_price, deposite' .
//                'FROM ' . $this->db->dbprefix('sales_items_taxes') . ' JOIN ' .
//                $this->db->dbprefix('detail_orders_massages') . ' USING (item_id, line) WHERE ' . $this->db->dbprefix('sales_items_taxes') . ".item_id = $sale_id");
...
//                $this->db->dbprefix('orders_item_kits') . ' USING (saleID, item_kitID, line) WHERE ' . $this->db->dbprefix('sales_item_kits_taxes') . ".saleID = $sale_id");
//        return $query->result_array();
//    }
//
