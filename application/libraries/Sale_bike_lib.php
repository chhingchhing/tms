...
//		{      
//                        $this->add_item($row->item_massage_id,$row->quantity_purchased, $row->discount_percent,$row->unit_price);
//		}
...
//	
        ...
        function add_item($item_id, $quantity = 1, $discount = 0, $price = null) {
//            echo $item_id; die(' item id'); //work
		//make sure item exists
		if (!$this->CI->bike_item->exists(is_numeric($item_id) ? (int) $item_id : -1)) {
			//try to get item id given an item_number
                    
			$item_id = $this->CI->bike_item->get_item_id($item_id);
//                        echo $item_id;die('die'); run for search by bike type
...
//	function add_item_kit($external_item_kit_id_or_item_number, $quantity = 1, $discount = 0, $price = null) {
...
//		} else {
//			$item_kit_id = $this->CI->bike_kit->get_item_kit_id($external_item_kit_id_or_item_number);
//		}
//
//		//make sure item exists
//		if (!$this->CI->bike_kit->exists($item_kit_id)) {
//			return false;
//		}
//
//		if ($this->CI->bike_kit->get_info($item_kit_id)->unit_price == null) {
...
//		if (!$this->CI->bike_kit->exists($kit_id))
//			return FALSE;
//
//		//Get All Items for Kit
//		$kit_items = $this->CI->bike_kit->get_info_item_kits($kit_id);
//
//		//Check each item
//		// Out of stock
//		foreach ($kit_items as $itm) {
//			$item = $this->CI->bike_item->get_info($itm->item_bike_id);
//			// var_dump($item);
//			$item_already_added = $this->get_quantity_already_added($itm->item_bike_id);
