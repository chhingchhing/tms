...
class Sale_ticket_lib
{
  	...
  	function __construct()
	{
		...
		$this->CI->load->model(array("sale_ticket"));
	}
	...
	*/
	function add_item($item_id,$quantity=1,$category,$discount=0,$price=null,$description=null,$serialnumber=null)
	{
		...
		if(!$this->CI->ticket_item->exists(is_numeric($item_id) ? (int)$item_id : -1))	
		{
		...
		foreach ($items as $item)
		{
			...
			if(isset($item['ticket_id']) && $item['ticket_id']==$item_id)
			{
				...
				if($this->CI->ticket_item->get_info($item_id)->description==$items[$updatekey]['description'] && $this->CI->ticket_item->get_info($item_id)->code_ticket==lang('sales_giftcard'))
					{
						return false;
					}
			} 
        ...
        $price_to_use = $this->CI->ticket_item->get_info($item_id)->sale_price;
		...
		array(
			'ticket_id'=>$item_id,
			'line'=>$insertkey,
			'name'=>$this->CI->ticket_item->get_info($item_id)->ticket_name,
			'description'=>$description!=null ? $description: $this->CI->ticket_item->get_info($item_id)->description,
			'quantity'=>$quantity,
            'discount'=>$discount,
	...
	function add_item_kit($external_item_kit_id_or_item_number,$quantity=1,$category,$discount=0,$price=null,$description=null)
	{
		...
		{
			foreach ($this->CI->item_kit_ticket->get_info($item_kit_id) as $item_kit_item)
			{ 
	...
	function get_item_id($line_to_get)
	{
		...
		foreach ($items as $line=>$item)
		{
			if($line==$line_to_get)
			{
				return isset($item['ticket_id']) ? $item['ticket_id'] : -1;
			}
		}
...
// Retrieve data of sale via id
	function copy_entire_sale($sale_id, $category)
	{
		...
		foreach($this->CI->sale_ticket->get_sale_items($sale_id)->result() as $row)
		{
			$this->add_item($row->ticketID,$row->quantity_purchased, $category, $row->discount_percent,$row->item_unit_price,$row->description);
		}
		...
		$this->CI->sale_lib->set_seat_no($this->CI->Ticket->get_seat_no($sale_id));
		$this->CI->sale_lib->set_item_vol($this->CI->Ticket->get_item_vol($sale_id));
	...
	function delete_item($line)
	{
		...
		if($this->CI->Giftcard->get_giftcard_id($this->CI->ticket->get_info($item_id)->description))
		{
			$this->CI->Giftcard->delete_completely($this->CI->ticket->get_info($item_id)->description);
		}
		unset($items[$line]);
		$this->CI->sale_lib->set_cart($items);
	}
